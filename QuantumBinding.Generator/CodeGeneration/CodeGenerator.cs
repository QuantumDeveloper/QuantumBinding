...
public abstract class CodeGenerator : CodeBlockGenerator
{
    ...
    {
        if (string.IsNullOrEmpty(AstContext.Module.FileHeader)) return;
            
        var header = AstContext.Module.FileHeader;

        PushBlock(CodeBlockKind.AutoGenerated);
        GenerateMultilineComment(header.Split(LineBreaks, StringSplitOptions.RemoveEmptyEntries), commentKind);
        PopBlock();
    }

    protected void GenerateSummary(string summary)
    {
        var lines = summary.Split(LineBreaks, StringSplitOptions.RemoveEmptyEntries);
