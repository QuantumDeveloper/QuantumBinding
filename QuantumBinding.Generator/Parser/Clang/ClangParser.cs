...
public unsafe class ClangParser : ICXCursorVisitor, IMetadataProvider
{
    ...
    private readonly HashSet<string> _visitedFunctions = new HashSet<string>();
    private readonly HashSet<string> _visitedMacros = new HashSet<string>();

    private QBIndex _index;
    private TranslationUnit _unit;
    private int _fieldPosition;

    public ClangParser(QBIndex index)
    {
        _index = index;
    }

    private QBTranslationUnit _translationUnit;
    ...
    public ParseResult Parse(TranslationUnit unit, string filePath)
    {
        ...
        var parseResult = ParseResult.Success;
        ...
        try
        {
            ...
            if (translationUnitResult != CXErrorCode.CXError_Success)
            {
                ...
                var numDiagnostics = _translationUnit.GetNumDiagnostics();
        ...
        {
            _translationUnit.DisposeTranslationUnit();
        }
    ...
    private List<string> GetArguments()
    {
        ...
        foreach (var include in _unit.Module.IncludeDirs)
        {
    ...
    private CXChildVisitResult Visit(QBCursor cursor, QBCursor parent, QBClientData data)
    {
    ...
    private CXChildVisitResult VisitEnum(QBCursor cursor, QBCursor parent, QBClientData data)
    {
        ...
        if (_visitedEnums.Contains(enumName))
        {
            return CXChildVisitResult.CXChildVisit_Continue;
        }
    ...
    private CXChildVisitResult VisitStruct(QBCursor cursor, QBCursor parent, QBClientData data)
    {
        ...
        CXChildVisitResult VisitStructFields(QBCursor cursor, QBCursor parent, QBClientData data)
        {
            ...
            {
                fieldName = "field" + this._fieldPosition;
            }
    ...
    private Dictionary<string, Enumeration> _enums = new Dictionary<string, Enumeration>();
    ...
    private CXChildVisitResult VisitVarDecl(QBCursor cursor, QBCursor parent, QBClientData data)
    {
        ...
        if (cursorType.IsConstQualifiedType() == 1)
        {
            ...
            if (!_enums.TryGetValue(enumName, out var @enum))
            {
    ...
    private CXChildVisitResult VisitTypedef(QBCursor cursor, QBCursor parent, QBClientData data)
    {
        ...
        {
            var @class = _unit.AllClasses.FirstOrDefault(x => x.Name == type.ToString());
            if (@class != null && @class.Name != spelling)
            {
                _unit.AddDummyType(spelling, @class.Name);
            }
            else if (@class == null && type.Kind == CXTypeKind.CXType_Enum)
            {
                _unit.AddDummyType(spelling, type.ToString());
            }
        ...
        if (type.Kind == CXTypeKind.CXType_Pointer)
        {
            ...
            if (pointee.Kind == CXTypeKind.CXType_FunctionProto)
            {
                ...
                CXChildVisitResult VisitFunctionProto(QBCursor cxCursor, QBCursor parent, QBClientData clientData)
                {
    ...
    private CXChildVisitResult VisitMacro(QBCursor cursor, QBCursor parent, QBClientData data)
    {
        ...
        if (_visitedMacros.Contains(spelling.ToString()))
        {
            return CXChildVisitResult.CXChildVisit_Continue;
        }

        _visitedMacros.Add(macroName);
        ...
        _translationUnit.Tokenize(extCur, out var tokens, out uint numTokens);
        ...
        for (var i = 0; i < tokens.Length; i++)
        {
            ...
            {
                Item = _translationUnit.GetTokenSpelling(token).ToString(),
    ...
    {
        declaration.Owner = _unit;
        _unit.AddDeclaration(declaration);
    }
